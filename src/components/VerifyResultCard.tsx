    ...
    <div className={`rounded-xl border p-5 ${style.classes}`}>
      ...
      {result.listing && (
        <div className="mt-4 rounded-lg bg-white/70 p-4 text-sm text-slate-800 space-y-1">
          {result.listing.isPlaceholder && (
            <p className="mb-2 inline-block rounded bg-slate-800 px-2 py-0.5 text-xs font-bold uppercase text-white">
              Demo data — not real
            </p>
          )}
          <p>
            <span className="font-semibold">Title:</span> {result.listing.title}
          </p>
          <p>
            <span className="font-semibold">Reference:</span> {result.listing.referenceNumber}
          </p>
          <p>
            <span className="font-semibold">Issuing body:</span> {result.listing.issuingBody}
          </p>
          <p>
            <span className="font-semibold">Status:</span> {result.listing.status}
          </p>
          {result.listing.closingDate && (
            <p>
              <span className="font-semibold">Closing date:</span> {result.listing.closingDate}
            </p>
          )}
          <p>
            <span className="font-semibold">Source:</span> {result.listing.sourceName}
          </p>
